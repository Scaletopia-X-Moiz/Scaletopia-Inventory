...
export function PushToEmailBisonCampaignButton({
  ...
  async function handleConfirmCampaign() {
    ...
    // every open of the dialog always starts from the pure auto-mapping
    // default, mirroring push-to-emailbison-button.tsx (the workspace
    // button)'s behavior. A prior version persisted this per (client,
    // "emailbison_companies") pair, but that meant a one-off tweak (e.g. a QA
    // static value) silently stuck around for every future push.
  ...
  // many of the about-to-be-pushed companies already look attached to a
  // different campaign in this workspace (lib/emailbison/push-to-emailbison.ts's
  // checkCompaniesCampaignConflicts — a proxy off our own platform_pushes.campaign_tag,
  ...
  async function checkCampaignConflicts() {
    ...
    try {
      const res = await fetch(
        `/api/emailbison/campaign-conflict-check?entity=companies&clientId=${selectedClient.id}&campaignId=${selectedCampaign.id}&${paramsStr}`
      );
