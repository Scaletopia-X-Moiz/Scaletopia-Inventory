...
for (const id of [1004, 1005]) {
  ...
  console.log("status:", del.status, JSON.stringify(del.json).slice(0, 300));
  console.log(`=== GET /api/campaigns/${id} (confirm gone) ===`);
  const get = await call("GET", `/api/campaigns/${id}`);
  console.log("status:", get.status, JSON.stringify(get.json).slice(0, 300));
}

console.log("\n=== Final campaign list ===");
const list = await call("GET", "/api/campaigns");
console.log(JSON.stringify(list.json?.data?.map(c => ({id: c.id, name: c.name})), null, 2));
