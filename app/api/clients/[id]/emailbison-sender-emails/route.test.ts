...
const { listSenderEmails } = vi.hoisted(() => ({ listSenderEmails: vi.fn() }));
vi.mock("@/lib/emailbison/client", async () => {
  ...
  return { ...actual, listSenderEmails };
});
...
describe("GET /api/clients/[id]/emailbison-sender-emails", () => {
  ...
  it("returns the workspace's sender emails", async () => {
    listSenderEmails.mockResolvedValue({
      senderEmails: [{ id: "1", name: "Jane Doe", email: "jane@example.com" }],
      page: 1,
      hasMore: false,
    });
    const res = await GET(makeRequest(), makeParams());
    ...
    expect(listSenderEmails).toHaveBeenCalledWith({ apiKey: "key", workspaceId: "ws" });
  });

  it("returns 502 when the EmailBison API call fails", async () => {
    listSenderEmails.mockRejectedValue(new Error("boom"));
    const res = await GET(makeRequest(), makeParams());
