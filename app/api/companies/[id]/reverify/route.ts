...
export async function POST(
  ...
  let outcome;
  try {
    outcome = await reverifyRecord("companies", id);
  } catch (err) {
    const message = (err as Error).message;
    await logActivity(
      "verify.reverify_one",
      { target: "companies", kind: "email", id, error: message, failed: true },
      user
    );
  ...
  await logActivity(
    "verify.reverify_one",
    { target: "companies", kind: "email", id, pending: outcome.pending },
    user
  );
